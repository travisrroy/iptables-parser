...
function parseRule(rule) {
  ...
  const [, sourceIp] = tryMatch(/\s--to-source\s([^\s]+)/, rule);
  ...
  const [, tcpFlags] = tryMatch(/\s--tcp-flags\s([^\s]+\s(SYN)?)/, rule);
  const [, match] = tryMatch(/\s-m\s((?!state|comment|limit|conntrack)[^\s]+)\s/, rule);
  ...
  const [, conntrack] = tryMatch(/\s-m\sconntrack\s--ctstate\s([^\s]+)/, rule);
...
function encodeRule({ 
  ...
  return map2Str('-A', chain) +
    ...
    map2Str('--tcp-flags', tcpFlags) +
    map2Str('-m state --state', state) +
    map2Str('-m conntrack --ctstate', conntrack) +
    map2Str('-m limit --limit', limit) +
    map2Str('-j', jump) +
    map2Str('-g', goto) +
    map2Str('--to-destination', destinationIp) +
    map2Str('--to-source', sourceIp) +
    map2Str('--log-prefix', logPrefix) +
    map2Str('--set-tos', tos) +
    map2Str('-m comment --comment', comment ? `"${comment}"` : null)
      .trim();
}
